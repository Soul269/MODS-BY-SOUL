...
def checkSpam(clientID):
	...
	if ID in counter: 
		...
		if counter[ID] == 3:
			...
			warnCount = warn(ID)
			with bs.Context(bsInternal._getForegroundHostActivity()):bs.screenMessage("DONOT SPAM ! PLS", transient=True, clients=[clientID])
			return False
			"""if warnCount < 2:
				bsInternal._chatMessage("{}, don't spam here!".format(name))
				with bs.Context(bsInternal._getForegroundHostActivity()):bs.screenMessage("DONOT SPAM 2 WARN", transient=True, clients=[clientID])
				
			else:
				spammers.pop(ID)		
				bsInternal._chatMessage("Warn limit exceeded. Kicking {} for spamming.".format(name))
				#bsInternal._chatMessage("ChatFilter Made By SOUL")
